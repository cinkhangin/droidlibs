...
const SponsorshipModal: React.FC<PaywallProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center p-6 bg-[#fafafa] backdrop-blur-md" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white border border-zinc-100 rounded-sm overflow-hidden flex flex-col shadow-[0_0_100px_-20px_rgba(74,222,128,0.2)]"
      onClick={(e) => e.stopPropagation()}>
        <div className="p-10">
          <div className="flex justify-between items-start mb-10">
            ...
            <button onClick={onClose} className="text-zinc-600 hover:text-emerald-500 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
            {tiers.map((tier) => (
              ...
              >
                {tier.tag && (
                  <span className="absolute -top-2 -right-2 bg-emerald-500 text-white text-[8px] font-bold px-2 py-0.5 uppercase tracking-tighter shadow-[0_4px_10px_rgba(74,222,128,0.3)]">
                    {tier.tag}
                  </span>
                )}
                <div className="flex justify-between items-center mb-2">
                  <span className={`text-[10px] font-bold uppercase tracking-widest ${selectedPlan === tier.id ? 'text-emerald-400' : 'text-zinc-500'}`}>
                    {tier.name}
                  </span>
                  <div className="text-right">
                    <span className="text-sm font-bold text-zinc-600">{tier.price}</span>
                    <span className="text-[9px] text-zinc-600 font-mono ml-1 uppercase">{tier.period}</span>
                  </div>
                </div>
